    ...
    <main class="p-6 bg-gray-100 min-h-screen">
        <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-lg">
            <div class="flex justify-between items-center mb-6">
                ...
                <RouterLink to="/transaction/create"
                    ...
                    <tbody v-if="transactions.length > 0">
                        <tr v-for="(transaction, index) in transactions" :key="index"
                            ...
                            <td class="py-3 px-6 text-left">
                                <RouterLink :to="{ path: '/transaction/' + transaction.id + '/edit' }"
                                ...
                                <button @click="deleteTransaction(transaction.id)" type="button"
                                    class="bg-red-500 hover:bg-red-600 text-white font-semibold py-2 px-4 rounded-lg shadow-md ml-2">
                                    Delete
                                </button>
                            </td>
                        </tr>
...
export default defineComponent({
    ...
    methods: {
        getTransactions() {
            axios.get('http://localhost:80/api/txn')
                .then(res => {
